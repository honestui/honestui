...
describe("handleError", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks()
    process.argv = ["node", "honestui", "add", "foo"]
  ...
  it("uses the package runner from the user agent", () => {
    ...
    expect(getPreviousMinorCommand("4.6.0", ["add", "foo"])).toBe(
      "pnpm dlx honestui@4.5.0 add foo"
    )
  })
  ...
  it("suggests the available previous minor before exiting", () => {
    expect(() => {
      handleError(
    ...
    expect(logger.error).toHaveBeenCalledWith("npx honestui@0.1.0 add foo")
