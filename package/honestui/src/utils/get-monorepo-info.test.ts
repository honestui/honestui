...
beforeEach(async () => {
  tmpDir = path.join(
    ...
    `honestui-monorepo-test-${Date.now()}`
  )
...
describe("formatMonorepoMessage", () => {
  it("should log the monorepo message with targets", () => {
    ...
    expect(
      allLogCalls.some((msg) => msg.includes("honestui init -c apps/web"))
    ).toBe(true)
    expect(
      allLogCalls.some((msg) => msg.includes("honestui init -c apps/docs"))
    ).toBe(true)
  ...
  it("should use the correct command name", () => {
    ...
    expect(
      allLogCalls.some((msg) =>
        msg.includes("honestui add [component] -c apps/web")
      )
    ).toBe(true)
