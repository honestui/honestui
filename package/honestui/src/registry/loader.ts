...
function resolveIncludePath(
  ...
  if (isUrl(includePath)) {
    throw new RegistryValidationError(
      `Invalid include "${includePath}" in ${registryFile}: remote includes are not supported by honestui build. Use a relative path to a registry.json file in the same repository.`,
      {
        registryFile,
        context: { includePath },
      }
...
function validateRegistryItemFiles(
  ...
  for (const file of item.files ?? []) {
    if (isUrl(file.path)) {
      throw new RegistryValidationError(
        `Invalid file path "${file.path}" for item "${item.name}" in ${registryFile}: remote file paths are not supported by honestui build.`,
        {
          registryFile,
