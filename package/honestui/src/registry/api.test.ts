...
const server = setupServer(
  ...
  http.get(`${REGISTRY_URL}/registries.json`, () => {
    return HttpResponse.json([
      {
        name: "@honestui",
        url: "https://honestui.com/r/styles/{style}/{name}.json",
        description: "The official honestui registry.",
      },
      {
        name: "@example",
...
describe("getRegistryItem", () => {
  it("should read and parse a valid local JSON file", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(tmpdir(), "honestui-test-"))
  ...
  it("should handle tilde (~) home directory paths", async () => {
    ...
    const tempFile = path.join(homeDir, "honestui-test-tilde.json")
    ...
    try {
      ...
      const tildeePath = "~/honestui-test-tilde.json"
...
describe("getRegistry", () => {
  ...
  it("should throw RegistryParseError with proper context", async () => {
    ...
    try {
      ...
      if (error instanceof RegistryParseError) {
        ...
        expect(error.suggestion).toContain(
          "https://honestui.com/schema/registry.json"
        )
  ...
  it("should distinguish between URL and registry name", async () => {
    ...
    const registryName = "@honestui"
    const registryUrl = "https://honestui.com/registry.json"

    // Mock for URL
    server.use(
      http.get(registryUrl, () => {
        return HttpResponse.json({
          name: "honestui-from-url",
          homepage: "https://honestui.com",
          items: [],
        })
      })
    )
    ...
    const mockConfig = {
      ...
      registries: {
        "@honestui": {
          url: "https://honestui.com/{name}.json",
        },
      },
    } as any
    ...
    expect(urlResult.name).toBe("honestui-from-url")
...
describe("getRegistriesConfig", () => {
  ...
  it("should handle complex registry configurations", async () => {
    ...
    const config = {
      style: "new-york",
      registries: {
        "@honestui": "https://honestui.com/r/styles/{style}/{name}.json",
    ...
    try {
      ...
      expect(result.registries?.["@honestui"]).toBe(
        "https://honestui.com/r/styles/{style}/{name}.json"
      )
