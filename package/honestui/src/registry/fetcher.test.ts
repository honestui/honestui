...
describe("fetchRegistry", () => {
  ...
  it("should send specific Accept and User-Agent headers", async () => {
    ...
    expect(acceptHeader).toBe(
      "application/vnd.honestui.v1+json, application/json;q=0.9"
    )
    expect(userAgentHeader).toBe("honestui")
  })
  ...
  it("should successfully fetch when the server requires the specific HonestUI Accept header (Content Negotiation)", async () => {
    server.use(
      http.get(`${REGISTRY_URL}/content-negotiation.json`, ({ request }) => {
        const accept = request.headers.get("accept")
        if (!accept?.includes("application/vnd.honestui.v1+json")) {
