...
describe("configWithDefaults", () => {
  ...
  it("should preserve user registries when merging", () => {
    ...
    expect(result.registries?.["@honestui"]).toBe(BUILTIN_REGISTRIES["@honestui"])
  })
  ...
  it("should override built-in registries if user provides same key", () => {
    const config = createConfig({
      registries: {
        "@honestui": "http://custom-honestui.com/{name}",
      },
    })

    const result = configWithDefaults(config)

    // User's @honestui should override the built-in one
    expect(result.registries?.["@honestui"]).toBe(
      "http://custom-honestui.com/{name}"
    )
  })
