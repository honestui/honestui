...
describe("searchRegistries", () => {
  it("should fetch and return registries in flat format", async () => {
    ...
    mockGetRegistry.mockImplementation(async (name: string) => {
      if (name === "@honestui" || name === "@honestui/registry") {
        return {
          name: "honestui",
    ...
    const results = await searchRegistries(["@honestui", "@custom"])
      ...
      items: [
        {
          ...
          description: "A button component",
          registry: "@honestui",
          addCommandArgument: "@honestui/button",
        },
        {
          name: "card",
          type: "registry:ui",
          description: "A card component",
          registry: "@honestui",
          addCommandArgument: "@honestui/card",
        },
        {
          name: "header",
          type: "registry:component",
  ...
  it("should apply search filter when query is provided", async () => {
    ...
    const results = await searchRegistries(["@honestui"], { query: "button" })

    expect(results.items).toHaveLength(1)
    expect(results.items[0].name).toBe("button")
    expect(results.items[0].registry).toBe("@honestui")
    expect(results.items[0].addCommandArgument).toBe("@honestui/button")
  ...
  it("should handle mixed registry names and URLs", async () => {
    ...
    mockGetRegistry.mockImplementation(async (nameOrUrl: string) => {
      if (nameOrUrl === "@honestui" || nameOrUrl === "@honestui/registry") {
        return {
          name: "honestui",
          homepage: "https://honestui.com",
          items: [
    ...
    // Should find the button from @honestui
    expect(results.items).toHaveLength(1)
    expect(results.items[0]).toMatchObject({
      name: "button",
