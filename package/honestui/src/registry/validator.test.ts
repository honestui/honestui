...
describe("validateRegistryConfig", () => {
  ...
  describe("built-in registries", () => {
    it("should not throw for @honestui since it's now a built-in registry", () => {
      expect(() => {
        validateRegistryConfig("@honestui", {
    ...
    it("should not throw for similar but different registry names", () => {
      expect(() => {
        validateRegistryConfig("@honestui-ui", {
          url: "https://example.com/{name}",
        })
      }).not.toThrow()

      expect(() => {
        validateRegistryConfig("@myhonestui", {
          url: "https://example.com/{name}",
        })
      }).not.toThrow()
    })
