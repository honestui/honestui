...
describe("resolveRegistryItemsFromRegistries", () => {
  ...
  it("should resolve non-registry items through @honestui registry", () => {
    const items = ["button", "card", "dialog"]
    const config = { registries: {} } as any

    const result = resolveRegistryItemsFromRegistries(items, config)

    // Non-prefixed items are resolved through the built-in @honestui registry
    expect(result).toEqual([
      "https://honestui.com/r/button.json",
      "https://honestui.com/r/card.json",
      "https://honestui.com/r/dialog.json",
    ])
    expect(setRegistryHeaders).toHaveBeenCalledWith({})
  })
  ...
  it("should handle mixed registry and non-registry items", () => {
    ...
    // Non-registry items (button, dialog) are resolved through the built-in @honestui registry
    expect(result).toEqual([
      "https://honestui.com/r/button.json",
      "https://v0.dev/chat/b/card/json",
      "https://honestui.com/r/dialog.json",
