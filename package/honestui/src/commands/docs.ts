...
const HONESTUI_BASE_URL = "https://honestui.com"
...
export const docs = new Command()
  ...
  .action(async (components, opts) => {
    try {
      ...
      const index = await getHonestUIRegistryIndex()
        ...
        if (!item) {
          logger.error(
            `Component ${highlighter.info(
              component
            )} not found in the Honest UI registry.`
          )
          process.exit(1)
        }
...
function normalizeLinks(links: Record<string, string>) {
  return Object.fromEntries(
    Object.entries(links).map(([key, value]) => [
      key,
      value.startsWith(HONESTUI_BASE_URL)
        ? `${HONESTUI_URL}${value.slice(HONESTUI_BASE_URL.length)}`
        : value,
    ])
  )
}
