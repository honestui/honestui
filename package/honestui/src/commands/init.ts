        ...
        if (options.template === "laravel" && !hasPackageJson) {
          ...
          logger.log(
            `  Please create a new app with ${highlighter.info(
              "laravel new --react"
            )} first then run ${highlighter.info("honestui init")}.`
          )
          logger.log(
            `  See ${highlighter.info(
              `${HONESTUI_URL}/docs/installation/laravel`
      ...
      if (options.preset !== undefined) {
        ...
        if (typeof presetArg === "string") {
          ...
          if (isUrl(presetArg)) {
            ...
            if (url.pathname === "/init" && presetArg.startsWith(HONESTUI_URL)) {
            ...
            // Preset codes no longer carry base. Honest UI only uses Base UI.
...
async function confirmBaseSwitch(existingStyle: string, resolvedBase: string) {
  ...
  logger.warn(
    `  Existing config uses ${highlighter.info(
      existingStyle
    )}. Honest UI now uses ${highlighter.info("Base UI")} presets only.`
  )
  logger.break()
