...
vi.mock("@/src/utils/get-config", () => ({
  ...
  DEFAULT_ICON_LIBRARY: "honestui/icons",
...
describe("runInit", () => {
  ...
  beforeEach(async () => {
    cwd = await mkdtemp(path.join(os.tmpdir(), "honestui-init-test-"))
...
describe("init options", () => {
  ...
  it("applies pointer to raw init urls", () => {
    const url = applyInitUrlOptions(
      new URL("https://honestui.com/init?preset=a0"),
      { pointer: true }
    )
  ...
  it("removes pointer from raw init urls when disabled", () => {
    const url = applyInitUrlOptions(
      new URL("https://honestui.com/init?preset=a0&pointer=true"),
      { pointer: false }
    )
