...
describe("preset commands", () => {
  ...
  it("rejects URLs", () => {
    expect(() =>
      decodePresetCode("https://honestui.com/create?preset=a0")
    ).toThrow("Invalid preset code: https://honestui.com/create?preset=a0")
  })
  ...
  it("does not open invalid preset codes", async () => {
    ...
    await expect(
      openCommand.parseAsync(["https://honestui.com/create?preset=a0"], {
        from: "user",
      })
    ).rejects.toThrow("process.exit:1")

    expect(logger.error).toHaveBeenCalledWith(
      "Invalid preset code: https://honestui.com/create?preset=a0"
    )
  ...
  it("resolves a project preset using the existing resolver", async () => {
    ...
    expect(logger.log).toHaveBeenCalledWith(
      expect.stringContaining("https://honestui.com/create?preset=b123")
    )
  ...
  it("prints fallback markers when resolving a project preset", async () => {
    ...
    expect(logger.log).toHaveBeenCalledWith(
      "  * Uses preset defaults for values not available as options on honestui create."
    )
  })
