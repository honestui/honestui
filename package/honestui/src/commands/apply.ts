      ...
      if (!preset) {
        ...
        await promptToOpenPresetBuilder({
          ...
          followUp: `Then run ${highlighter.info(
            "honestui apply --preset <preset>"
          )} with the preset code or preset URL from honestui.com.`,
          prompt: !options.yes,
        })
  ...
  if (!knownPresetNames.includes(preset)) {
    logger.error(
      `Invalid preset: ${highlighter.info(
        preset
      )}.\nUse one of the available presets: ${knownPresetNames.join(", ")} \nor build your own at ${highlighter.info(`${HONESTUI_URL}/create`)}`
    )
    logger.break()
    process.exit(1)
  }
...
export function resolveApplyInitUrl(
  ...
  if (isUrl(preset)) {
    ...
    if (url.pathname === "/init" && preset.startsWith(HONESTUI_URL)) {
...
function getInitCommand(preset?: string) {
  if (!preset) {
    return "honestui init"
  }

  return `honestui init --preset ${quoteShellArg(preset)}`
}
