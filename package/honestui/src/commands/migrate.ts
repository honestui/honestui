...
export const migrateOptionsSchema = z.object({
  ...
  migration: z
    ...
    .refine(
      ...
      {
        message:
          "You must specify a valid migration. Run `honestui migrate --list` to see available migrations.",
      }
    )
    .optional(),
  ...
  .action(async (migration, migratePath, opts) => {
    try {
      ...
      if (!options.migration) {
        throw new Error(
          "You must specify a migration. Run `honestui migrate --list` to see available migrations."
        )
      }
