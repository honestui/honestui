...
import { REGISTRY_URL, HONESTUI_URL } from "@/src/registry/constants"
...
export function resolveCreateUrl(
  ...
  const url = new URL(`${HONESTUI_URL}/create`)
...
export function resolveInitUrl(
  ...
  return `${HONESTUI_URL}/init?${params.toString()}`
}
...
function isHonestUIInitUrl(url: string) {
  try {
    return new URL(url).pathname === "/init" && url.startsWith(HONESTUI_URL)
  } catch {
    return false
  }
}
