---
title: Styling
description: How Honest UI uses CSS variables, Tailwind CSS, and state colors.
---

## Overview

Honest UI uses CSS variables for colors and Tailwind CSS for component classes.
...
```bash
npx honestui@latest add colors-zinc
```
...
Honest UI adds extra variables for state-based UI such as alerts, badges, validation, and status messages:
...
To customize Honest UI, change the values in `:root` and `.dark`.
