...
Examples cover status, loading, actions, promise state, and varied toast heights.
...
Examples cover the animated toast style, state variants, promise handling, action buttons, and placement.
...
`ToastProvider` installs the viewport and accepts the documented position. `toastManager` exposes the creation, update, dismissal, promise, and action methods shown in the examples; each toast accepts a status variant and visible content. Keep the provider mounted once near the application root.

Toasts are supplemental feedback. Persist errors or results that users may need after the timeout, and do not use a toast as the only confirmation of a destructive action.
