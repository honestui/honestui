...
Every Honest UI chart component accepts a `chartConfig` object. It maps each data key to its display metadata, **labels**, **colors** (with theme support), and optional **icons** shown in tooltips and legends.
...
Each key (e.g. `harvested`, `packed`) must match a data key in your dataset. Its type:
