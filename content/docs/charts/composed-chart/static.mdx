...
description: Our composed chart for combining bars and lines in one visualization.
image: /og/composed-chart.png
links:
  github: https://github.com/honestui/honestui/blob/main/registry/default/charts/composed-chart.tsx
...
Use a composed chart when bars and lines explain different measures over the same categories, such as procedure volume and wait time. Keep the number of series small so the mixed encodings remain easy to read.
...
```tsx
import { ComposedChart, type ChartConfig } from "honestui/charts";
```
...
<ComposedChart xDataKey="month" data={data} config={chartConfig}>
  ...
  <ComposedChart.Bar dataKey="procedures" variant="gradient" isClickable />
  <ComposedChart.Line dataKey="waitTime" strokeVariant="dashed" isClickable>
...
ECharts renders to a `<canvas>`, so these children never mount as DOM, the root reads their props and compiles them into the ECharts option object. Same JSX, same presence semantics (omit a part and it doesn't render), same behavior; the children are just config, not live DOM nodes.
...
<ApiHeading>ComposedChart</ApiHeading>
...
<ApiTable>
  ...
  <ApiRow name="config" type="ChartConfig" required>
    Defines every bar and line series. Each key matches a data key, with a `label` and a per-theme `colors` array. Same contract as every Honest UI chart, see <Link href="/docs/charts/chart-config">Chart Config</Link>.
  </ApiRow>
