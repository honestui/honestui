---
title: Area Chart
description: Our composable area chart with polished fills, strokes, and interactions.
...
ECharts renders to a `<canvas>`, so these children never mount as DOM. The root reads their props and compiles them into the ECharts option, same JSX, same presence semantics (omit a part and it won't render), same behavior, just declarative config instead of live DOM nodes.

The `config` is the same contract as every Honest UI chart, each key maps a data key to a `label` and a per-theme `colors` array. See <Link href="/docs/charts/chart-config">Chart Config</Link> for the full shape. Colors resolve from CSS variables at runtime, so dark mode just works.
