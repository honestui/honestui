---
title: Line Chart
description: Our composable line chart with polished strokes, markers, and interactions.
image: /og/line-chart.png
links:
  github: https://github.com/honestui/honestui/blob/main/registry/default/charts/line-chart.tsx
...
ECharts renders to a `<canvas>`, so these children never mount as DOM. The root reads their props and compiles them into the ECharts option object. Same JSX, same presence semantics (omit a part and it doesn't render), same behavior, the children are declarative config, not live DOM nodes.
...
<ComponentPreview className="mb-0" title="Hospital census" name="ex-glowing-desktop-echarts-line-chart"  />
<ComponentPreview title="Train punctuality" name="ex-glowing-mobile-echarts-line-chart"  />
...
<ApiHeading>LineChart</ApiHeading>
...
<ApiTable>
  ...
  <ApiRow name="config" type="ChartConfig" required>
    Defines the chart's series, each key matches a data key with a `label` and per-theme `colors` array. Same contract as every Honest UI chart; see <Link href="/docs/charts/chart-config">Chart Config</Link>.
  </ApiRow>
