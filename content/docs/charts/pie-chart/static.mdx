---
title: Pie Chart
description: Show a simple part-to-whole relationship with a small number of categories.
...
The root compiles its children into an ECharts option and renders the plot on a canvas. The `config` prop maps each sector key to its label and theme colors. See <Link href="/docs/charts/chart-config">Chart Config</Link> for the complete shape.
