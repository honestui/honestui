---
title: Pie Chart
description: Our pie chart with donut, gradient, and pop-out selection variants.
image: /og/pie-chart.png
links:
  github: https://github.com/honestui/honestui/blob/main/registry/default/charts/pie-chart.tsx
...
```tsx
<PieChart data={data} dataKey="visitors" nameKey="browser" config={chartConfig}>
...
The `config` is the same contract as every Honest UI chart, each key maps a sector name to a `label` and a per-theme `colors` array. See <Link href="/docs/charts/chart-config">Chart Config</Link> for the full shape. Colors resolve from your CSS variables at runtime, so dark mode works with no extra wiring.
...
<ApiHeading>PieChart</ApiHeading>
...
<ApiTable>
  ...
  <ApiRow name="config" type="ChartConfig" required>
    Defines the chart's sectors. Each key matches a value from your `nameKey` field, with a `label` and a per-theme `colors` array. Same contract as every Honest UI chart, see <Link href="/docs/charts/chart-config">Chart Config</Link>.
  </ApiRow>
